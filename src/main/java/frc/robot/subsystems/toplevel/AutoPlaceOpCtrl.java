...
public class AutoPlaceOpCtrl extends OperationCtrl {
    ...
    private void stateLookingForTag() throws BadParameterTypeException, MissingParameterException {
        ...
        if (!a.isPresent()) {
            MessageLogger logger = getRobotSubsystem().getRobot().getMessageLogger() ;
            logger.startMessage(MessageType.Error).add("AutoPlaceOpCtrl:LookingForTag: DriverStation did not return an alliance").endMessage();
            logger.endMessage();
        }
        else {
            int tag = getRobotSubsystem().getFieldData().getGridTag(a.get(), getOper().getAprilTag());
    ...
    private void visionNextStep() throws BadParameterTypeException, MissingParameterException {
        ...
        if (!a.isPresent()) {
            MessageLogger logger = getRobotSubsystem().getRobot().getMessageLogger() ;
            logger.startMessage(MessageType.Error).add("AutoPlaceOpCtrl:VisionNextStep: DriverStation did not return an alliance").endMessage();
            logger.endMessage();
        }
        else {        
            getRobotSubsystem().getSwerve().enableVision(false);
            target_pose_ = getRobotSubsystem().getFieldData().getGridPose(a.get(), getOper().getAprilTag(), getOper().getSlot());
    ...
    private void stateWaitingForVision() throws BadParameterTypeException, MissingParameterException {
        ...
        if (!a.isPresent()) {
            MessageLogger logger = getRobotSubsystem().getRobot().getMessageLogger() ;
            logger.startMessage(MessageType.Error).add("AutoPlaceOpCtrl:LookingForTag: WaitForVision did not return an alliance").endMessage();
            logger.endMessage();
        }
        else {        
            int tag = getRobotSubsystem().getFieldData().getGridTag(a.get(), getOper().getAprilTag());
